...
my $message = << '&EOT&';
Usage: perl <path>/GPOS2RPOS.pl [-I <input file (bam)>] [-MS <microsatellte location file>] [-REF <Reference file (fasta)>] [-h]
...
my %option_hash = ("I" => "/dev/stdin", "REF" => $bwa_ref, "MQ" => $mq_cutoff, "LL" => $len_cutoff1, "ML" => $len_cutoff2, "FL" => $flanking_len_cutoff, "SL" => $S_length_cutoff, "BQ" => $q_score_cutoff, "ISN" => $indel_S_num_cutoff, "SW" => $SW_alignment, "GO" => $d, "GE" => $e);
...
while(<IN>){
	...
	my $cmd = "samtools view -F 1024 -F 0x400 $BAM \"$MS_pos\"|perl $SRC/GPOS2RPOS.pl -CHR $chr -GP1 $start -GP2 $last $option_str";
