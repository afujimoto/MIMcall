...
my %call_option = ("BD" => $call_prms{BLOOD_MIN_DEPTH}, "CD" => $call_prms{CANCER_MIN_DEPTH}, "BL" => $call_prms{BLOOD_L}, "CL" => $call_prms{CANCER_L}, "ER" => "$SRC/$call_prms{ERROR_RATE_TABLE}", "VAF" => "$call_prms{VAF}", "BN" => $call_prms{BN}, "CN" => $call_prms{CN},);
